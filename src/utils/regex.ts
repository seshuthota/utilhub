...
export function parseAiResponse(response: string) {
    ...
    if (regexMatches.length > 0) {
        ...
        const sortedMatches = regexMatches.sort((a, b) => {
            const getRegexScore = (m: RegExpMatchArray) => {
                const patt = m[1];
                let score = 0;
            ...
            const aInBlock = (a.index !== undefined ? response.substring(0, a.index).includes('```') : false);
            const bInBlock = (b.index !== undefined ? response.substring(0, b.index).includes('```') : false);
...
export function explainRegex(pattern: string) {
    ...
    const explanation: { token: string; label: string; description: string }[] = [];
    let i = 0;
