...
import { Copy, Share2, ChevronDown, ExternalLink, RefreshCcw, ShieldCheck, Clock, Database, Braces, Code } from 'lucide-react';
...
export default function ActionToolbar({ content, currentToolId, onCopy }) {
    ...
    useEffect(() => {
        ...
        if (currentToolId !== 'json' && (content.startsWith('{') || content.startsWith('['))) {
            recs.push({
                toolId: 'json',
    ...
    const handleCopy = () => {
        if (onCopy) {
            ...
            showToast('Copied to clipboard', 'success', content.substring(0, 30) + '...');
        }
    };
        ...
        <div className={styles.toolbar}>
            ...
            {recommendations.length > 0 && (
                <div className={styles.dropdownWrapper}>
                    ...
                    >
                        <Share2 size={14} /> Send to... <ChevronDown size={14} className={isOpen ? styles.rotate : ''} />
                    </button>
                            ...
                            <div className={styles.dropdown}>
                                ...
                                {recommendations.map((tool, idx) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <Icon size={16} className={tool.isDetected ? styles.detectedIcon : ''} />
                                            <div className={styles.itemContent}>
                                                <span className={styles.itemLabel}>{tool.label}</span>
                                                {tool.isDetected && <span className={styles.detectedBadge}>Magic Match ✨</span>}
                                            </div>
                                        </button>
                                    );
                                })}
