...
export function useUrlState(key: string, initialValue: string = ''): [string, (newValue: string) => void] {
    ...
    const [value, setValue] = useState<string>(getInitialState);
    ...
    const setSyncedValue = (newValue: string) => {
        setValue(newValue);
        updateUrl(newValue);
    };
