        ...
        <div className={styles.container}>
            <header className={styles.header}>
                ...
                <div className={styles.controls}>
                    <button className="btn-secondary" onClick={() => setInput(EXAMPLE_YAML)}>
                        Load Example
                    </button>
                    <label className="btn-primary" style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem' }}>
                        <Upload size={16} /> Upload YAML
                        <input type="file" onChange={handleFileUpload} accept=".yaml,.yml" style={{ display: 'none' }} />
                    </label>
                </div>
            </header>
                        ...
                        <div className={styles.resultsArea}>
                            {isValid === true && (
                                <div className={styles.valid}>
                                    ...
                                    <div>
                                        Valid YAML Syntax
                                        {k8sStatus === 'valid' && <div style={{ fontSize: '0.9rem', opacity: 0.9 }}>✓ Valid Basic Kubernetes Resource</div>}
                                        {k8sStatus === 'invalid' && <div style={{ fontSize: '0.9rem', color: 'var(--error-color)' }}>⚠ Invalid Kubernetes Structure</div>}
                                    </div>
                                </div>
                            )}
                                    ...
                                    <ul className={styles.errorList}>
                                        {errors.map((err, idx) => (
                                            <li key={idx} className={styles.errorItem}>
                                                {err.line !== 'N/A' && <span className={styles.errorLocation}>Line {err.line}:</span>}
                                                {err.message}
                                            </li>
                                        ))}
                                    </ul>
