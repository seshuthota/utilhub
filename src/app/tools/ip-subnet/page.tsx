...
export default function IpSubnetTool() {
    ...
    const calculation = useMemo<IpCalculation | null>(() => {
        try {
            // Validate IP
            ...
            const subnetMaskLong = cidr === 0 ? 0 : (-1 << hostBits);
    ...
    function getIpClass(firstOctet: number) {
    ...
    function isPrivate(ipLong: number) {
                    ...
                    <div className={styles.field} style={{ flex: '0 0 120px' }}>
                        ...
                        >
                            {Array.from({ length: 33 }, (_, i) => (
                                <option key={32 - i} value={(32 - i).toString()}>/{32 - i}</option>
                            ))}
                        </select>
                    </div>
                </div>
...
function ResultItem({ label, value, copy }: ResultItemProps) {
    return (
