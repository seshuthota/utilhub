...
export default function ChmodCalculator() {
    ...
    const calculateValues = (perms: Permissions) => {
        let oct = '';
        let sym = '-';

        (Object.keys(perms) as (keyof Permissions)[]).forEach(scope => {
            const p = perms[scope];
    ...
    const handleCheckboxChange = (scope: keyof Permissions, type: keyof PermissionSet) => {
        setPermissions(prev => {
            const newPerms = {
                ...prev,
                [scope]: {
