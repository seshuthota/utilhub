...
export default function ConfigConverter() {
    ...
    const stringifyContent = (obj: any, format: string) => {
        switch (format) {
            case "json":
    ...
    const convert = useCallback((content: string, from: string, to: string) => {
        if (!content.trim()) {
            setOutput("");
            setError(null);
            return;
    ...
    const handleSwap = () => {
        const oldSource = sourceFmt;
        const oldTarget = targetFmt;
        setSourceFmt(oldTarget);
        setTargetFmt(oldSource);
        setInput(output);
    };
