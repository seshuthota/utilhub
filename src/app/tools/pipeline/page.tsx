...
function ToolSelector({ onSelect, onClose }: ToolSelectorProps) {
...
function StepOptions({ step, onUpdate, onRemove }: StepOptionsProps) {
...
function PipelineStep({ step, index, onUpdate, onRemove, isActive, onClick }: PipelineStepProps) {
    const adapter = getToolAdapter(step.toolId);
...
function ResultsPanel({ results, currentStep, error }: ResultsPanelProps) {
...
function ExamplePipelines({ onLoad }: ExamplePipelinesProps) {
                        ...
                        <div className={styles.stepsSection}>
                            ...
                            {pipeline.steps.length > 0 && (
                                <div className={styles.flowIndicator}>
                                    <ArrowRight size={20} />
                                    <span>
                                        Input → {pipeline.steps.map((s: any) => s.toolId).join(" → ")} →
                                        Output
                                    </span>
                                </div>
                            )}
