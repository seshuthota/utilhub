...
interface ConversionCategory {
    ...
    units: string[];
    base?: number;
    toBase: (v: number, unit: string) => number;
    fromBase: (v: number, unit: string) => number;
}
...
const conversions: Record<string, ConversionCategory> = {
    ...
    time: {
        ...
        toBase: (v, unit) => {
            const factors: Record<string, number> = { ms: 1, s: 1000, min: 60000, hr: 3600000, days: 86400000 };
...
export default function UnitsTool() {
    const [category, setCategory] = useState<string>('bytes');
    const [value, setValue] = useState<number>(1024);
    const [fromUnit, setFromUnit] = useState<string>('KB');
