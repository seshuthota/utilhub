...
export default function ApiTesterTool() {
    ...
    const [headers, setHeaders] = useState<Param[]>([{ key: 'Content-Type', value: 'application/json', active: true }]);
    const [auth, setAuth] = useState<Auth>({ type: 'none', username: '', password: '', token: '' });
    ...
    useEffect(() => {
        try {
            ...
            if (saved) {
                ...
                loaded.forEach((c: Collection) => expanded[c.id] = true);
    ...
    const handleSaveToHistory = (req: any) => {
        const entry = {
            id: Date.now(),
            method: req.method,
            url: req.url,
    ...
    const loadFromHistory = (entry: any) => {
        ...
        setHeaders(
            Array.isArray(entry.headers)
                ? entry.headers
                : Object.entries(entry.headers || {}).map(([key, value]) => ({ key, value: String(value), active: true }))
        );
        setBody(entry.body || '');
    ...
    const loadFromCollection = (req: SavedRequest) => {
    ...
    const handleImportCurl = () => {
        ...
        setMethod(result.method as string);
        setUrl(result.url || '');
    ...
    const removeParam = (idx: number) => updateParams(params.filter((_, i) => i !== idx));
    const updateParamField = (idx: number, field: keyof Param, value: any) => {
    ...
    const updateHeaderField = (idx: number, field: keyof Param, value: any) => {
