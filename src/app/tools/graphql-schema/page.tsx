...
export default function GraphqlSchemaTool() {
    ...
    const [mode, setMode] = useState<"ts" | "json">("ts");
    const [error, setError] = useState<string | null>(null);

    const convertToTs = (sdl: string) => {
        try {
            const ast = parse(sdl);
            ...
            visit(ast, {
                ...
                EnumTypeDefinition(node) {
                    const values = node.values
                        ?.map((v) => `  ${v.name.value} = "${v.name.value}"`)
                        .join(",\n");
    ...
    const process = useCallback((sdl: string, currentMode: "ts" | "json") => {
        if (!sdl.trim()) {
            setOutput("");
            setError(null);
            return;
                        ...
                        <div className={styles.paneHeader}>
                            ...
                            >
                                ...
                                <select
                                    ...
                                    value={mode}
                                    onChange={(e) => setMode(e.target.value as "ts" | "json")}
                                >
