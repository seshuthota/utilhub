...
function SizeWarning({ fileSize, processing }: { fileSize: number; processing: boolean }) {
    ...
    if (isOverLimit) {
        return (
            <div className={styles.warningBanner} style={{ background: "var(--error-bg)", borderColor: "var(--error-color)", color: "var(--error-color)" }}>
                <AlertTriangle size={16} />
                <span>File too large ({formatBytes(fileSize)}). Maximum size is {formatBytes(MAX_FILE_SIZE)}.</span>
            </div>
        );
    }
    ...
    if (isHeavy) {
        return (
            <div className={styles.warningBanner} style={{ background: "rgba(255, 165, 0, 0.15)", borderColor: "orange", color: "orange" }}>
                <AlertTriangle size={16} />
                <span>Large file ({formatBytes(fileSize)}). Using WebAssembly acceleration.</span>
                {processing && <span className={styles.processing}><Loader2 size={14} className="animate-spin" /> Processing...</span>}
            </div>
        );
    }
            ...
            <div className={styles.grid}>
                <div className={styles.controls}>
                    <div className={styles.uploadBox} onClick={() => fileInputRef.current?.click()}>
                        ...
                        {fileSize > 0 && <span className={styles.fileSize}>{formatBytes(fileSize)}</span>}
                    </div>
                    ...
                    {image && (
                        <>
                            <div className={styles.filterSection}>
                                ...
                                {(activeFilter === "brighten" || activeFilter === "saturate") && (
                            ...
                            <button onClick={processImage} className={styles.processBtn} disabled={isProcessing}>
                                {isProcessing ? (
